...
summarise(max_gdp= max(gdpPercap), min_gdp= min(gdpPercap)) %>% 
  knitr::kable(format = "markdown", padding=2, caption= "MAX and MIN gdps per capita")
...
means %>% 
  knitr::kable(format = "markdown", padding=2, caption= "Trimmed and weightened means")
means %>% ggplot(aes(wmean, tm_lifeExp, color= year  ) )+ geom_point()+ geom_path()

```

+ How is life expectancy changing over time on different continents?
#Writing the same function in two ways (for practice)
```{r}
ggplot(gapminder, aes(lifeExp, year , size= gdpPercap, color= continent)) +
    facet_grid(~ continent) +
    geom_point(alpha= 0.3)
```
+ Report the absolute and/or relative abundance of countries with low life expectancy over time by continent: Compute some measure of worldwide life expectancy – you decide – The benchmark for the analysis is the mean life expectancy (59.47444). I will devide the results in **high** life expectancy if it is higher than the mean or **low**
